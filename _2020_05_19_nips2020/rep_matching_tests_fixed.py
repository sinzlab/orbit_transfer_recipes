...
for dataset_cls in (
    ...
    for seed in (42,
        ...
        experiments[
            Description(
                name=dataset_cls + ": Clean (fixed)", seed=seed
            )
        ] = Experiment(
        ...
        experiments[
            Description(
                name=dataset_cls + ": Noise Augmented + Repr Matching (Euclid; clean only; fixed)", seed=seed
            )
        ] = Experiment(
            dataset=dataset.ImageDatasetConfig(
                comment=dataset_cls,
        ...
        experiments[
            Description(
                name=dataset_cls + ": Noise Augmented + Adv Regression (fixed)", seed=seed
            )
        ] = Experiment(
            dataset=dataset.ImageDatasetConfig(
                comment=dataset_cls,
        ...
        transfer_experiments[
            Description(
                name=dataset_cls
                     + ": Noise Augmented (fixed) -> Transfer",
                seed=seed,
            )
        ] = TransferExperiment(
            [
                experiments[
                    Description(
                        name=dataset_cls + ": Noise Augmented (fixed)",
                        seed=seed,
                    )
                ],
                transfer,
        ...
        transfer_experiments[
            ...
            [
                experiments[
                    Description(
                        name=dataset_cls + ": Noise Augmented + Repr Matching (Euclid; clean only; fixed)",
                        seed=seed,
                    )
                ],
                transfer,
