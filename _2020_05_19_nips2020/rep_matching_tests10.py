...
for dataset_cls in (
    ...
    for seed in (42,
        ...
        experiments[
            Description(
                name=dataset_cls + ": Noise Augmented + Repr Matching (Euclid; clean only; 10)", seed=seed
            )
        ] = Experiment(
            ...
            trainer=trainer.TrainerConfig(
                comment="Noise Augmented + Repr. Matching (Euclid; clean only; 10)",
        ...
        transfer_experiments[
            Description(
                name=dataset_cls
                     + ": Noise Augmented + Repr Matching (Euclid; clean only; 10) -> Transfer",
                seed=seed,
            )
        ] = TransferExperiment(
            [
                experiments[
                    Description(
                        name=dataset_cls + ": Noise Augmented + Repr Matching (Euclid; clean only; 10)",
                        seed=seed,
                    )
                ],
                transfer,
