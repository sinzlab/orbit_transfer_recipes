...
for dataset_cls in (
    ...
    for seed in (42,
        ...
        experiments[
            Description(
                name=dataset_cls + ": Noise Augmented + Repr Matching (Euclid; clean only; 4)", seed=seed
            )
        ] = Experiment(
            ...
            trainer=trainer.TrainerConfig(
                comment="Noise Augmented + Repr. Matching (Euclid; clean only; 4)",
        ...
        transfer_experiments[
            Description(
                name=dataset_cls
                     + ": Noise Augmented + Repr Matching (Euclid; clean only; 4) -> Transfer",
                seed=seed,
            )
        ] = TransferExperiment(
            [
                experiments[
                    Description(
                        name=dataset_cls + ": Noise Augmented + Repr Matching (Euclid; clean only; 4)",
                        seed=seed,
                    )
                ],
                transfer,
