...
for bias in (
    ...
    transfer_method = "KnowledgeDistillation"
    alpha = 1.0
    ...
    for softmax_temp in (0.5, 1.0, 2.0, 4.0, 8.0, 10.0, 16.0):
        ######## step 3: transfer
        trainer_options = {
            "L2": {
                "optimizer_options": {
        ...
        experiments[
            Description(
                name=f"{dataset_sub_cls} Transfer ({transfer_method}: {softmax_temp}) {bias[1]}",
                seed=seed,
            )
        ] = Experiment(
        ...
        transfer_experiments[
            Description(
                name=f"Transfer {transfer_method}: {softmax_temp} ({bias[0]}->{bias[1]};{bias[2]})",
                seed=seed,
            )
        ] = TransferExperiment(
            [
